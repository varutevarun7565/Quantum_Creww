...
object JeevanRepository {
    ...
    // Real device: http://10.14.2.178:8000/  ← your PC's current IP
    ...
    private fun SosStateDto.toDomain(): SosState {
        val userLoc = when {
            ...
            user?.lat != null && user.lng != null -> LatLng(user.lat, user.lng)
            else -> null
        }
        ...
        val amb = ambulance?.let {
            AmbulanceInfo(
                ...
                type   = it.type ?: "ALS",
                phone  = it.phone ?: "",
                lat    = ambLoc?.lat ?: it.lat ?: 0.0,
                lng    = ambLoc?.lng ?: it.lng ?: 0.0,
            )
        }
        val hosp = hospital?.let {
            HospitalInfo(
                ...
                phone      = it.phone ?: "",
                beds       = it.beds ?: 8,
                lat        = hospLoc?.lat ?: it.lat ?: 0.0,
                lng        = hospLoc?.lng ?: it.lng ?: 0.0,
            )
        }
        return SosState(
            active    = active,
    ...
    suspend fun triggerSos(lat: Double, lng: Double): SosState = withContext(Dispatchers.IO) {
        try {
            val res = api.triggerSos(SosTriggerRequest(lat, lng))
            if (res.isSuccessful) {
                val state = res.body()?.state?.toDomain() ?: buildMockSosState(lat, lng)
                mockState = state
                return@withContext state
            }
        } catch (e: Exception) {
            Log.w("JeevanRepo", "Backend unreachable, using mock: ${e.message}")
        }
        val mock = buildMockSosState(lat, lng)
        mockState = mock
        mock
    }
