...
class JeevanViewModel : ViewModel() {
    ...
    private fun onLocationReceived(context: Context, loc: Location) {
        Log.d("JeevanViewModel", "[Android Side] GPS Location received: Lat=${loc.latitude}, Lng=${loc.longitude}")

        val finalLat = loc.latitude
        val finalLng = loc.longitude

        val latLng   = LatLng(finalLat, finalLng)
        val address  = reverseGeocode(context, finalLat, finalLng)
        val hospitals = JeevanRepository.getMockNearbyHospitals(finalLat, finalLng)
        _uiState.update {
    ...
    // ────────────────────────────────────────────
    fun triggerSOS() {
        val loc = _uiState.value.userLoc
        if (loc == null) {
            _uiState.update { it.copy(error = "Waiting for live GPS location. Please ensure Location is enabled.") }
            return
        }
